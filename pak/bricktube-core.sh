#!/bin/sh
# Brick Tube core: keyboard search -> results grid -> resolve -> on-device
# http/https proxy -> tplayerdemo (CedarX HW decode), driven via a stdin FIFO so
# stop is a clean "quit" (video layer torn down — no grey screen). MENU stops,
# A/B pause, volume never stops playback. Stopping a video returns to results.
# Launched by a per-OS launch.sh wrapper that owns BIN/LD_LIBRARY_PATH/PATH and
# the BRICKTUBE_PANEL_W/H env for the rectfix shim.
DIR="$(dirname "$0")"
BIN="${BIN:-/mnt/SDCARD/Videos}"   # shared binaries (avoid 40MB SD-card duplication)
