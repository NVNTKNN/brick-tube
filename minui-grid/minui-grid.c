...
struct ListItem
{
    ...
    char *name;
    // path to the grid thumbnail image (minui-grid)
    char *thumb;
    // lazily loaded, pre-scaled thumbnail surface (minui-grid)
    SDL_Surface *thumb_surface;
    // whether a thumbnail load was already attempted (minui-grid)
    bool thumb_tried;
...
struct ListState *ListState_New(const char *filename, const char *format, const char *item_key, const char *confirm_text, const char *default_background_image, const char *default_background_color, struct AppState *app_state)
{
    ...
    if (strcmp(format, "text") == 0)
    {
        ...
        state->items = calloc(item_count, sizeof(struct ListItem));
    ...
    else
    {
        for (size_t i = 0; i < item_count; i++)
        {
            ...
            state->items[i].thumb = thumb ? strdup(thumb) : NULL;
...
// --- minui-grid: 3-wide thumbnail grid ---
#define GRID_COLS 3

// keep the scroll window aligned to whole rows and containing the selection
void GridState_Scroll(struct ListState *list, int visible_rows)
{
    int row = list->selected / GRID_COLS;
    int first_row = list->first_visible / GRID_COLS;
    if (row < first_row)
        first_row = row;
    ...
    if (first_row < 0)
        first_row = 0;
    list->first_visible = first_row * GRID_COLS;
    list->last_visible = list->first_visible + visible_rows * GRID_COLS;
    if ((size_t)list->last_visible > list->item_count)
        list->last_visible = (int)list->item_count;
}
...
void handle_input(struct AppState *state)
{
    ...
    {
        if (state->list_state->selected >= GRID_COLS)
        {
            state->list_state->selected -= GRID_COLS;
            GridState_Scroll(state->list_state, max_row_count);
            state->redraw = 1;
        }
    }
    else if (PAD_justRepeated(BTN_DOWN))
    {
        int last = (int)state->list_state->item_count - 1;
        int target = state->list_state->selected + GRID_COLS;
        if (target <= last)
        {
        ...
        else if (state->list_state->selected / GRID_COLS < last / GRID_COLS)
        {
            // partial last row: land on the last item
            state->list_state->selected = last;
            GridState_Scroll(state->list_state, max_row_count);
            state->redraw = 1;
        }
    }
    ...
    {
        if (state->list_state->selected < (int)state->list_state->item_count - 1)
        {
            state->list_state->selected += 1;
            GridState_Scroll(state->list_state, max_row_count);
            state->redraw = 1;
...
void draw_screen(SDL_Surface *screen, struct AppState *state, int ow, bool should_draw_background_image)
{
    ...
    // --- minui-grid: 3-wide thumbnail grid in place of list rows ---
    ...
    int cell_h = thumb_h + title_h + SCALE1(6);

    for (int i = state->list_state->first_visible, j = 0; i < state->list_state->last_visible; i++, j++)
    {
        struct ListItem *it = &state->list_state->items[i];
        int col = j % GRID_COLS;
        int row = j / GRID_COLS;
        int x = gutter + col * (cell_w + gutter);
        int y = grid_top + row * (cell_h + gutter);

        // selection: white frame around the thumbnail
        if (i == state->list_state->selected)
        {
            SDL_Rect hi = {x - SCALE1(3), y - SCALE1(3), cell_w + SCALE1(6), thumb_h + SCALE1(6)};
        ...
        if (!it->thumb_tried)
        {
            if (it->thumb != NULL && access(it->thumb, F_OK) != -1)
            {
                ...
                if (raw != NULL)
                {
                    SDL_Surface *conv = SDL_ConvertSurface(raw, screen->format, 0);
                    SDL_FreeSurface(raw);
                    if (conv != NULL)
                    {
                        SDL_Surface *scaled = SDL_CreateRGBSurfaceWithFormat(
                            0, cell_w, thumb_h, conv->format->BitsPerPixel, conv->format->format);
                        if (scaled != NULL && SDL_BlitScaled(conv, NULL, scaled, NULL) == 0)
                        {
                            it->thumb_surface = scaled;
                        }
                        else if (scaled != NULL)
                        {
                            SDL_FreeSurface(scaled);
        ...
        if (it->thumb_surface != NULL)
        {
            SDL_BlitSurface(it->thumb_surface, NULL, screen, &cell);
        }
        else
        {
            SDL_FillRect(screen, &cell, SDL_MapRGB(screen->format, 40, 40, 40));
        }

        // title line under the thumbnail
        char truncated[256];
        GFX_truncateText(state->fonts.medium, it->name, truncated, cell_w, SCALE1(4));
        SDL_Color tc = (i == state->list_state->selected) ? COLOR_WHITE : COLOR_GRAY;
        SDL_Surface *text = TTF_RenderUTF8_Blended(state->fonts.medium, truncated, tc);
        if (text != NULL)
        {
            SDL_Rect tp = {x, y + thumb_h + SCALE1(4), text->w, text->h};
            SDL_BlitSurface(text, NULL, screen, &tp);
            SDL_FreeSurface(text);
        }
    }
...
int main(int argc, char *argv[])
{
    ...
    // minui-grid: max_row_count means visible GRID ROWS (2 x 3 cells on 1024x768)
    state.max_row_count = 2;

    // validate selection and compute initial visible window (then row-align it)
    ListState_InitView(state.list_state, state.max_row_count);
    GridState_Scroll(state.list_state, state.max_row_count);
    ...
    while (!state.quitting && !signal_exit_code)
    {
        ...
        if (state.redraw)
        {
            ...
            // headless render check: dump the first frame and exit (dev only)
            const char *shot = getenv("MINUI_GRID_SHOT");
            if (shot != NULL)
            {
